...
function seasonView(season) {
  ...
  const cards = tasks.map(({ plant, task }) => `<article class="task-card"><div class="task-thumb-wrap"><div class="image-fallback">Photo<br>to add</div><img class="task-thumb" data-plant-image src="${imageUrl(plant.image)}" alt="${plant.name}" loading="lazy" decoding="async"></div><div><span class="task-meta">${task.timing}</span><h3>${plant.name} — ${task.title}</h3><p>${task.details}</p></div><a class="open-plant" href="#plants/${plant.id}/season/${season}">View plant →</a></article>`).join('');
