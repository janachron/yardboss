...
function plantView(selectedId, returnHash = '#home') {
  const cards = plants.map((plant, index) => {
    ...
    return `<article class="plant-card" id="plant-${plant.id}"><img src="${plant.image}" alt="${plant.name}"><div class="card-content"><span class="plant-number">${String(index + 1).padStart(2, '0')} / ${String(plants.length).padStart(2, '0')}</span><h2>${plant.name}</h2><div class="care-list">${tasks}</div></div></article>`;
  }).join('');
...
function seasonView(season) {
  ...
  const cards = tasks.map(({ plant, task }) => `<article class="task-card"><img class="task-thumb" src="${plant.image}" alt="${plant.name}"><div><span class="task-meta">${task.timing}</span><h3>${plant.name} — ${task.title}</h3><p>${task.details}</p></div><a class="open-plant" href="#plants/${plant.id}/season/${season}">View plant →</a></article>`).join('');
